...
@Component({
  selector: 'app-grafica-nueva',
  imports: [],
  templateUrl: './grafica-nueva.component.html',
  styleUrl: './grafica-nueva.component.css'
})
export class GraficaNuevaComponent {
  ...
  ngAfterViewInit() {
    ...
    // Agregar leyenda
    let legend = mainPanel.plotContainer.children.push(am5stock.StockLegend.new(this.root, {
      stockChart: stockChart
    }));

    // Agregar indicador de Bollinger Bands
  ...
  ngOnDestroy() {
    // Limpiar el gráfico cuando se destruye el componente
    if (this.root) {
      this.root.dispose();
    }
  }
